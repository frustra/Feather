	...
	public void inject(CustomClassNode node) {
		for (MethodNode method : (List<MethodNode>) node.methods) {
			if (method.name.equals(PlayerSocketHandlerClass.playerLeft.name) && method.desc.equals(PlayerSocketHandlerClass.playerLeft.desc)) {
				...
				iList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Feather.class.getName().replace('.', '/'), "playerLeft", Type.getMethodDescriptor(Type.VOID_TYPE, new Type[] { Type.getType(Object.class) })));

...
@HookingPass(2)
public class SendEntityMessageMethod extends MethodProvider {
	public boolean match(FilamentClassNode node) throws BadHookException {
		return node.matches("CommandEntity");
	}

	public boolean match(FilamentClassNode node, MethodNode m) throws BadHookException {
		Type[] args = new Type[] { Type.getObjectType(Hooks.getClassName("FormatableComponent")) };
	...
	public void complete(FilamentClassNode node, MethodNode m) {
		Hooks.set("CommandEntity.sendMessage", m);
	}
}
